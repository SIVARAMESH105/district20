 ...
 * @package Auth
 */
class ManageChapterDirectoriesController extends Controller
{
    /**
    ...
    {  
        ...
        $data['title'] = "Manage Chapter Directory";
        return view('manage_chapter_directories.index', $data);  
    }
    /**
     ...
     */
    public function getChapterDirectoriesListAjax(Request $request)
    {
        return datatables()->of((new Directories)->getAllChapterDirectoriesAjax())
                        ->addColumn('action', function($directories){
                            $action = '<div class="action-btn"><a class="btn btn-info btn-xs" title="Edit" href="'.route('manage-chapter-directories-edit', $directories->id) .'"><i class="fas fa-pencil-alt"></i></a>';
                            $action .=' <a class="btn btn-danger btn-xs delete-directory" href="javascript:;" data-id="'.$directories->id.'" data-url="'.route('manage-chapter-directories-delete', $directories->id) .'"><i class="fas fa-trash"></i></a></div>';
                            return $action;
                        })                       
                        ->rawColumns(['action'])
                        ->make(true);
    ...
    {   
        $data['title'] = "Manage Chapter Directory - Add";        
        $data['chapters'] = Chapter::all();
        return view('manage_chapter_directories.add', $data);
    }
    /**
    ...
    public function save(DirectoriesCreateRequest $request)
    {
        ...
        if((new Directories)->createDirectoryWithType($info)) {         
            $request->session()->flash('success', "New Chapter Directory Created Successfully.");
            return redirect(route('manage-chapter-directories'));
        } else {            
            $request->session()->flash('error', "Nothing to update (or) unable to update.");
            return redirect(route('manage-chapter-directories'))->withInput();
        }
    }
    /**
    ...
    public function edit(Request $request)
    {
        ...
        $data['title'] = "Manage Chapter Directory - Edit";
        return view('manage_chapter_directories.edit', $data);
    }
    /**
    ...
    public function update(DirectoriesUpdateRequest $request)
    {
        ...
        if((new Directories)->updateDirectory($info)) {
            $request->session()->flash('success', "Chapter Directory Updated Successfully.");
            return redirect(route('manage-chapter-directories'));
        } else {            
            $request->session()->flash('error', "Nothing to update (or) unable to update.");
            return redirect(route('manage-chapter-directories'))->withInput();
    ...
    {
        $data['directories'] = Directories::all();
        $data['title'] = "Chapter Directory";
        return view('manage_chapter_directories.view', $data);
    }
    /**
    ...
    public function import(Request $request)
    {
        if ($request->isMethod('post')) {
            $data = CommonHelper::importContractorElseChapterDirectory($request, 4);
            if($data) {
                 $request->session()->flash('success', "Chapter Directory Imported Successfully.");
                return redirect(route('manage-chapter-directories'));
            } else {
        ...
        $data['title'] = "Manage Chapter Directory - Import";
        return view('manage_chapter_directories.import', $data);        
    }
}