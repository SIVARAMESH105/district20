  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <div class="card card-primary">
        <form role="form" id="add-user" action="{{ route('manage-user-save') }}" name="add-user" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="card-body">
            ...
            @else
              <input type="hidden" name="chapter" value="{{ Auth::user()->chapter_id }}">
            @endif
            <div class="form-group">
              <label for="rnumber">Registration Number</label>
              <input type="rnumber" class="form-control" id="rnumber" name="rnumber" placeholder="Enter Registration Number" value="{{ old('rnumber') }}">
            </div>
            <div class="form-group">
              ...
              <div class="form-group">
                ...
                <select type="user_role" class="form-control" id="user_role" name="user_role">
                  ...
                  @foreach($roles as $role)
                    <option value="{{ $role->id }}" {{ $role->id==old('user_role') ? 'selected' : ''}}>{{ $role->role_name }}</option>
                  @endforeach
                </select>
              </div> 
            @else
